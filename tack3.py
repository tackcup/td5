...
settings = {
    ...
    "man2":"ตั้งข้อความ",
    "man3":"ตั้งข้อความ",
    "message":"บัญชีนี้ได้การป้องกัน💢โดย แทค Bot Self ระบบได้ทำการบล็อคคุณอัตโนมัติ",
    "comment":" แทค Bot Self",
    "userAgent": [
...
mulai = time.time() 
dangerMessage = ["แทคเสียใจด้วยครับ","โชคดีครับ","ชีวิตยังมีงวดหน้าเสมอ",".winebot",".kickall","mayhem","kick on","makasih :d","!kickall","nuke","บิน",".???","Kickall","บินไปดิ","Kickword","kickword","!kickword","/kickall"]
dangerMessage2 = ["ว่างัยจ๊ะทิรักของเค้า 5555","คร้าบบ","ว่างัยน่ะ!!.. รักเค้าหรออ อิอิ","ว่างัยเอ๋ยย เค้ารอฟังอยู่","เค้ามาแล้วคร้าบบ","แอบรักเค้าอยู่หราาา","แอบชอบเค้าหราา","รักเค้าหรอ","คิดถึงเค้ามากเลยหรอ"]
...
def listgrup():
    listGrup =  """💢คำสั่งตั้งค่า แทคBotSelf💢
...
def helpkicker():
    helpKicker = """⏳คำสั่งค้นหา แทคBotSelf⏳
...
def helpsetting():
    helpSetting = """⚓คำสั่งกลุ่ม แทคBotSelf⚓
🍁 กลุ่ม ➠ ข้อมูลกลุ่ม
🍁 ไอดีกลุ่ม ➠ไอดีห้อง
🍁 ชื่อกลุ่ม ➠ ชื่อห้อง
...
def helptexttospeech():
    helpTextToSpeech =   """⚡คำสั่งเลียนแบบแทคBotSelf⚡
🔁 เลียนแบบ on/off 
🔁 เลียนแบบ 
🔁 เพิ่มเลียนแบบ (@) 
🔁 ลบเลียนแบบ (@)"""
...
#===============หาชื่อล่อนหน===============================================================#
                    ...
                    ki.sendMessage(to, "แทค BotSelfยังอยู่ครับ")
                elif text.lower() == 'เทส':
                    line.sendMessage(to, "แทค BotSelfกำลังโหลด:▒...0%")
                    ...
                    line.sendMessage(to, " 🔱แทค BotSelf ยังอยู่ครับ✔")
                    ...
                    start = time.time()
                    line.sendMessage(to,"ความเร็วของ แทค Bot Self...")
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["speed","sp","Speed","สปีด","!speed","/speed"]:
                    line.sendText(msg.to,"แรงขนาดนี้ แทคไลค์เลย👍")
                ...
                if msg.text in ["555","5555","55555","55+","55++","555+","555++","5555+","5555++"]:
                    line.sendText(msg.to,"แทคว่าไปหาหมอบ้างน่ะ อิอิ")				
                if msg.text in dangerMessage:
                    random.choice(Rfu).kickoutFromGroup(receiver,[sender])
                    random.choice(Rfu).sendText(msg.to,"ตรวจพบคำสั่งของบอทลบกลุ่ม  แทค Bot Self จำเป็นต้องนำออกเพื่อความปลอดภัยของสมาชิก (｀・ω・´)")										
        if op.type == 17:
          if settings["Wc"] == True:
                ...
                if RfuCctv['cyduk'][op.param1]==True:
                    if op.param1 in RfuCctv['point']:
                        ...
                        if Name in RfuCctv['sidermem'][op.param1]:
                            ...
                            pref=['สวัสดีครับคนอ่าน➡','ตร๊ะ!..เอ๋!!..😝➡','หวัดดีจร้า💖➡','แทคเห็นนะ อิอิ..😝➡','แอบทำมัย? มาคุยกันหน่อย➡','แอบแฟนเล่นหราา💨➡','แทครู้นะว่าคัยอ่าน👉➡','เล่นซ่อนแอบหรอคร้าบบ👻➡','มาคุยกับเค้าหน่อย➡','นั่นแน่ แอบหราาา➡']
